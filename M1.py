...
def model_1(string_input, mip_gap, time_limit, scaler, periods=None, case=0, iterate=False,
    ...
    #R1) Todos los insumos de un día deben ser cortados
    MVTPI.addConstrs((quicksum(X[k, t] for k in K) == q[t] for t in T))

    #R2) Relación inventarios iniciales
    MVTPI.addConstrs((S[f, t, u] == S0[f][u] - D[f, t, u] for f in F for t in [T[0]] for u in range(t + 1, t + delta - 1)))

    #R3) Relación inventarios NO iniciales
    MVTPI.addConstrs((S[f, t, u] == S[f, t - 1, u] - D[f, t, u]
                      for f in F for t in T[1:] for u in range(t + 1, t + delta - 1)))

    #R4) Inicialización de inventarios recién producidos
    MVTPI.addConstrs((S[f, t, t + delta - 1] == quicksum(a[f][k]*X[k, t] for k in K) - D[f, t, t + delta - 1]
                      for f in F for t in T))

    #R5)Definición de demandas en función del precio
    MVTPI.addConstrs((quicksum(D[f, t, u] for u in range(t, t + delta))
                      == alfa[f][t] - beta[f][t]*P[f, t]
                      for f in F for t in T))
   
    #R6)Mermas en primer período
    MVTPI.addConstrs(S0[f][1] - D[f, 1, 1] == L[f, 1] for f in F)

    #R7) Mermas en otros períodos
    MVTPI.addConstrs((S[f, t - 1, t] - D[f, t, t] == L[f, t] for f in F for t in T[1:]))
    
    #R8) Limite de precio  (estas se agregan en definiciones de variables con lb y ub)
    MVTPI.addConstrs(P[f, t] <= alfa[f][t]/beta[f][t] for f in F for t in T if beta[f][t] != 0)

    #R9) Compatibilidad de precio. Para cada período y relación de precio (f_quants), la suma producto es >= 0
    ...
    #R10) Evitar canibalizacion de precios entre subproductos.
    ...
    # Demandas FIFO
    #R11) Inventario inicial en t de producto final F que vence en u=t,t+1...t+delta-1
    inventario_incial_set = [(f, t, u) for f in F for t in T for u in range(t, t + delta)]
    S_inicial = MVTPI.addVars(inventario_incial_set, vtype=GRB.CONTINUOUS, lb=0,  name = 's')
    MVTPI.addConstrs(S_inicial[f, 1, u] == S0[f][u] for u in range(1, delta) for f in F)
    MVTPI.addConstrs(S_inicial[f, t+1, u] == S[f, t, u] for f in F for t in T[:-1] for u in range(t+1, t+delta) )

    # R12) Generar FIFO para asignacion de venta de inventario inicial
    #B = MVTPI.addVars(inventario_incial_set, vtype=GRB.BINARY, name = 'BINARIA1')
    #M = 1e9
    #MVTPI.addConstrs(
    #    M*B[f, t, u] >= S_inicial[f, t, u] - D[f, t, u]  for f in F for t in T for u in range(t, t+delta)
    #)
    ...
    #MVTPI.addConstrs(
    #    M*(1-B[f, t, u]) >= D[f, t+1, u] for f in F for t in T[:-1] for u in range(t+1, t+delta)
    #)
    ...
    if iterate:
            ...
            produccion_w = None
            S_inicial_ =  {(f,t,u): S_inicial[f, t, u].X for (f,t,u) in inventario_incial_set}
            ...
            return production, price, pattern_use, inventario, demands, L, S_inicial_, demanda
